...
fishers = fisher_desi + fisher_cosvis + fisher_hirax + fisher_hetdex
names = ['DESI', 'CosVis_nw', 'CosVis_3pb', 'CosVis_hw', 
         'HIRAX_nw', 'HIRAX_3pb', 'HIRAX_hw', 'HETDEXdz03']
...
P.errorbar(_zc, mu, yerr=(dm, dp), 
           ...
           elinewidth=1.5, ecolor=col_hrx_light)
...
P.errorbar(_zc, mu, yerr=(dm, dp), 
           ...
           elinewidth=1.5, ecolor=col_cv_light)
...
if ii == 0:
    ...
    P.legend(loc='upper right', frameon=False, prop={'size': 'large'})
else:
    P.ylim((5e-4, 0.8))
    P.ylabel(r"$\sigma_{D_A} / D_A$", fontsize=15, labelpad=10)
    P.legend(loc='upper left', frameon=False, prop={'size': 'large'})
